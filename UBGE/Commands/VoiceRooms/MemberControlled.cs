...
namespace UBGE.Commands.VoiceRooms
{
    [Group("sala"), OnlyUBGE, ConnectedToMongo]
    ...
    public class MemberControlled : BaseCommandModule
    {
        ...
        public async Task DelMembroNaSalaAsync(CommandContext ctx, DiscordMember membro = null)
        {
            ...
            if (ctx.Message.MentionedUsers.Count > 1)
            {
                embed.WithAuthor($"❎ - Remova um membro por vez!", null, Values.logoUBGE);
                embed.WithColor(Program.Bot.Utilities.RandomColorEmbed());
            ...
            else
            {
                if (resultadoSalas[0].idDoDono == membro.Id)
                {
                    embed.WithAuthor("❎ - Você não pode remover a si mesmo!", null, Values.logoUBGE)
                        .WithColor(Program.Bot.Utilities.RandomColorEmbed());
                ...
                if (lista.Contains(membro.Id))
                {
                    ...
                    embed.WithAuthor($"✅ - O usuário: \"{Program.Bot.Utilities.DiscordNick(membro)}#{membro.Discriminator}\" foi removido da lista branca!", null, Values.logoUBGE);
                    embed.WithColor(Program.Bot.Utilities.RandomColorEmbed());
                ...
                {
                    embed.WithAuthor($"✅ - O usuário: \"{Program.Bot.Utilities.DiscordNick(membro)}#{membro.Discriminator}\" não foi encontrado na lista branca!", null, Values.logoUBGE);
                    embed.WithColor(Program.Bot.Utilities.RandomColorEmbed());
        ...
        public async Task IdentificarSalaAsync(CommandContext ctx, DiscordChannel sala = null)
        {
            ...
            {
                embed.WithColor(Program.Bot.Utilities.HelpCommandsColor())
                    .WithAuthor("Digite só um ID de um canal por vez!", null, Values.logoUBGE)
            ...
            if (resultadoSalas.Count != 0 && ctx.Guild.GetChannel(sala.Id) != null)
            {
                ...
                foreach (ulong membros in ultimaRespostaSala.idsPermitidos)
                {
                    if (membros != ultimaRespostaSala.idDoDono)
                        str.Append($"{Program.Bot.Utilities.MemberMention(await ctx.Guild.GetMemberAsync(membros))}, ");
                }
                ...
                embed.WithAuthor($"Informações do canal de voz: \"{sala_.Name}\"", null, Values.logoUBGE)
                    .WithColor(Program.Bot.Utilities.RandomColorEmbed())
                    .WithThumbnailUrl(ctx.Member.AvatarUrl)
                    .WithDescription($"Dono: {Program.Bot.Utilities.MemberMention(await ctx.Guild.GetMemberAsync(ultimaRespostaSala.idDoDono))}\n\n" +
            ...
            {
                embed.WithAuthor($"❎ - Esta sala não existe ou você colocou o ID errado!", null, Values.logoUBGE);
                embed.WithColor(Program.Bot.Utilities.RandomColorEmbed());
    ...
    {
        [Command("create"), Aliases("criar", "summon"), CreateYourRoomHere]

        public async Task CriarCanalAsync(CommandContext ctx)
        {
            DiscordChannel cliqueAqui = ctx.Guild.GetChannel(Values.Chats.channelCliqueAqui);
