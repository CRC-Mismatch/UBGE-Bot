...
namespace UBGE.Commands.General
{
    ...
    public sealed class StaffControlled : BaseCommandModule
    {
        ...
        public async Task VerCargoAsync(CommandContext ctx, DiscordRole cargo = null)
        {
            ...
            embed.AddField("Admin?:", cargo.CheckPermission(Permissions.Administrator) == PermissionLevel.Allowed ? "**Sim**" : "**Não**", false)
                ...
                .WithAuthor($"Informações do cargo: \"{cargo.Name}\"", null, Values.logoUBGE)
        ...
        [Command("viewemoji"), Aliases("ve", "veremoji"), UBGEStaff]
        ...
        public async Task VerEmojiAsync(CommandContext ctx, string emojiNome = null)
        {
            ...
            {
                embed.WithColor(Program.Bot.Utilities.HelpCommandsColor())
                    .WithAuthor($"Digite o nome do emoji!", null, Values.logoUBGE)
                    .WithThumbnailUrl(ctx.Member.AvatarUrl)
            ...
            DiscordEmoji emojo = Program.Bot.Utilities.FindEmoji(ctx, emojiNome);
            ...
            embed.WithColor(Program.Bot.Utilities.RandomColorEmbed())
                .WithAuthor($"Informações do Emoji: \"{emojo.Name}\" - ({emojo.Id})", null, Values.logoUBGE)
        ...
        public async Task BotStatusAsync(CommandContext ctx)
        {
            ...
            embed.WithColor(Program.Bot.Utilities.RandomColorEmbed())
                .WithAuthor("Status do Bot:", null, Program.Bot.DiscordClient.CurrentUser.AvatarUrl)
                ...
                .WithThumbnailUrl(Values.csharpLogo)
                .WithTimestamp(DateTime.Now)
                .WithFooter($"Comando requisitado pelo: {Program.Bot.Utilities.DiscordNick(ctx.Member)}", iconUrl: ctx.Member.AvatarUrl);
        ...
        public async Task DesligarAsync(CommandContext ctx)
        {
            await ctx.TriggerTypingAsync();
        ...
        [Command("listarservidores"), Aliases("listarservidor"), UBGEStaff]
        ...
        public async Task ListarServidorAsync(CommandContext ctx)
        {
            ...
            DiscordEmbedBuilder embed = new DiscordEmbedBuilder
            {
                Author = new DiscordEmbedBuilder.EmbedAuthor { Name = "Servidores em que estou:", IconUrl = Values.logoUBGE },
    ...
    public sealed class MongoStaffControlled : BaseCommandModule
    {
        ...
        public async Task ExportarAsync(CommandContext ctx, [RemainingText] string colecao = null)
        {
            ...
            {
                builder.WithAuthor("Digite o nome da coleção!", null, Values.logoUBGE)
                    .WithColor(Program.Bot.Utilities.RandomColorEmbed())
            ...
            {
                builder.WithAuthor("Essa coleção não existe!", null, Values.logoUBGE)
                    .WithColor(Program.Bot.Utilities.RandomColorEmbed())
                    .WithDescription(":thinking:")
                    .WithThumbnailUrl(ctx.Member.AvatarUrl)
                    .WithFooter($"Comando requisitado pelo: {Program.Bot.Utilities.DiscordNick(ctx.Member)}", iconUrl: ctx.Member.AvatarUrl)
            ...
            builder.WithAuthor($"Coleção exportada com sucesso de local.{colecao} para {colecao}.json", null, Values.logoUBGE)
                .WithColor(Program.Bot.Utilities.RandomColorEmbed())
        ...
        public async Task ImportarAsync(CommandContext ctx, [RemainingText] string colecao = null)
        {
            ...
            if (ctx.Message.Attachments.ToList().Count != 0)
            {
                if (ctx.Message.Attachments.ToList().First().Url != null)
                {
                    ...
                    builder.WithAuthor($"Coleção importada com sucesso de {colecao}.json para local.{colecao}", null, Values.logoUBGE)
                        .WithColor(Program.Bot.Utilities.RandomColorEmbed())
                        .WithDescription(Program.Bot.Utilities.FindEmoji(ctx, ":UBGE:"))
                        .WithThumbnailUrl(ctx.Member.AvatarUrl)
                        .WithTimestamp(DateTime.Now)
        ...
        public async Task CriarColecaoAsync(CommandContext ctx, [RemainingText] string collection = null)
        {
            ...
            if (!string.IsNullOrWhiteSpace(nomeDaColecao))
            {
                builder.WithAuthor("Essa coleção já existe!", null, Values.logoUBGE)
                    .WithColor(Program.Bot.Utilities.RandomColorEmbed())
                    .WithDescription(":thinking:")
                    .WithThumbnailUrl(ctx.Member.AvatarUrl)
                    .WithFooter($"Comando requisitado pelo: {Program.Bot.Utilities.DiscordNick(ctx.Member)}", iconUrl: ctx.Member.AvatarUrl)
            ...
            builder.WithAuthor($"Coleção criada com sucesso em: local.{collection}", null, Values.logoUBGE)
                .WithColor(Program.Bot.Utilities.RandomColorEmbed())
                .WithDescription(Program.Bot.Utilities.FindEmoji(ctx, ":UBGE:"))
                .WithTimestamp(DateTime.Now)
                .WithThumbnailUrl(ctx.Member.AvatarUrl)
        ...
        public async Task ExcluirColecaoAsync(CommandContext ctx, [RemainingText] string collection = null)
        {
            ...
            builder.WithAuthor($"Coleção excluída com sucesso em: local.{collection}", null, Values.logoUBGE)
                .WithColor(Program.Bot.Utilities.RandomColorEmbed())
                .WithDescription(Program.Bot.Utilities.FindEmoji(ctx, ":UBGE:"))
                .WithThumbnailUrl(ctx.Member.AvatarUrl)
                .WithTimestamp(DateTime.Now)
        ...
        public async Task VerColecoesAsync(CommandContext ctx)
        {
            ...
            if (listas.Count() == 0)
            {
                embed.WithAuthor("Não existe coleções no banco de dados!", null, Values.logoUBGE)
                    .WithColor(Program.Bot.Utilities.RandomColorEmbed())
                    .WithDescription(":thinking:")
                    .WithThumbnailUrl(ctx.Member.AvatarUrl)
                    .WithFooter($"Comando requisitado pelo: {Program.Bot.Utilities.DiscordNick(ctx.Member)}", iconUrl: ctx.Member.AvatarUrl)
            ...
            embed.WithAuthor("Coleções atuais:", null, Values.logoUBGE)
                .WithColor(Program.Bot.Utilities.RandomColorEmbed())
                .WithDescription(Program.Bot.Utilities.FindEmoji(ctx, ":UBGE:"))
